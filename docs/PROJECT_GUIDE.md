...
- **IN PROGRESS — "Roast mode" personality toggle (paused mid-session, not yet built).**
  Idea: user saw a screenshot of another app ("mumu") where the AI reacts to a logged
  expense with a sassy/savage roast in Filipino (Taglish), e.g. teasing "1000 pesos
  for a carwash?? may hand and foot spa ba yan?" instead of a plain friendly reply.
  Wants this as an **opt-in feature flag**, not a replacement — two independent
  per-chat toggles, same pattern as `getCurrency`/`setCurrency`:
  - **Tone**: `FRIENDLY` (default, current behavior) | `SAVAGE` (playful roast/tease
    about the purchase — sassy best-friend energy, NOT genuinely insulting the
    person or any protected trait, only teases the spending choice itself)
  - **Language**: `EN` (default) | `TL` (Filipino/Taglish)
  - Proposed commands: `/settone friendly|savage`, `/setlanguage en|tl` (naming not
    finalized — pick whatever reads cleanest)

  **What was found before pausing** (`src/gemini-bot.en.gs`):
  - The `reaction` field is generated by Gemini inside the JSON the parser returns,
    in **two** places that both need the tone/language instructions threaded through:
    1. `parseAndReactWithGemini(text, userName, currency, members)` — text-message
       path, prompt around line ~811-847, `reaction` field in the return schema
       (~line 842). Called from two sites: the multi-line loop (~line 724) and the
       single-line flow (~line 770).
    2. `handleReceiptPhoto(msg, chatId)` — receipt-photo path, prompt around
       line ~1319-1369, `reaction` field (~line 1367).
  - Both prompts currently hardcode "friendly" tone/English implicitly (e.g. "You are
    a friendly group expense assistant" at ~line 812). Need to parameterize the
    tone/language instruction text and pass `getTone(chatId)` / `getLanguage(chatId)`
    into both functions (mirroring how `getCurrency`/`getMembers` are already passed in).
  - `reaction` is rendered via `escapeHtml(parsed.reaction || "")` at the end of the
    reply in both call sites (~line 787 and ~line 1407) — no changes needed there,
    the escaping already handles any script/emoji Gemini returns.
  - Guardrail to build in: the SAVAGE tone prompt should explicitly say to keep it
    playful/exaggerated and never genuinely hateful, insulting, or targeting the
    person's identity — only tease the purchase/spending decision. This keeps the
    feature fun without the model going overboard.

  **Not yet done:** constants (`DEFAULT_TONE`, `DEFAULT_LANGUAGE`, maybe a `TONES`/
  `LANGUAGES` map like `CURRENCIES`), `getTone`/`setTone`/`getLanguage`/`setLanguage`
  helpers, the two new commands, threading params through both Gemini prompts,
  `/help` + `/start` + README + this guide's command table update.
